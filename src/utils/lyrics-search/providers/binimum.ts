...
const VALID_TIMING_TYPES: ReadonlySet<SyncType> = new Set<SyncType>([
  "syllable",
  "word",
  "line",
]);
...
function buildSearchUrl(query: LyricsSearchQuery): URL {
  ...
  if (hasNonEmptyString(query.track))
    url.searchParams.set("track", query.track.trim());
  if (hasNonEmptyString(query.artist))
    url.searchParams.set("artist", query.artist.trim());
  if (hasNonEmptyString(query.album))
    url.searchParams.set("album", query.album.trim());
  if (
...
function deriveSyncType(timingType: string): SyncType {
  if (VALID_TIMING_TYPES.has(timingType as SyncType))
    return timingType as SyncType;
  return "line";
}

function mapResponseToResult(result: BinimumSearchResult): LyricsSearchResult {
  const album = hasNonEmptyString(result.album_name)
    ? result.album_name
    : undefined;
  const payload: LyricsSearchPayload = {
    kind: "deferred-ttml",
    fetchUrl: result.lyricsUrl,
  };
  ...
  try {
    ...
    if (response.status >= 500) {
      throw new LyricsSearchError(
        "binimum",
        `Binimum search returned ${response.status}`,
      );
    }
    if (!response.ok) {
      throw new LyricsSearchError(
    ...
    for (const candidate of body.results) {
      if (
        !candidate ||
        typeof candidate.lyricsUrl !== "string" ||
        candidate.lyricsUrl.length === 0
      )
        continue;

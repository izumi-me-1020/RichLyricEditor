...
const TITLE = "Recover Your Work ・ RichLyricEditor";
const DESCRIPTION =
  "Grab the backup RichLyricEditor saved in this browser and pick up where you left off.";
      ...
      <div className="w-full max-w-lg flex flex-col items-center text-center gap-5">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <p className="text-sm text-composer-text-secondary leading-relaxed">
            RichLyricEditor saves your work automatically as you go. This page
            grabs that backup and downloads it as a file. Open RichLyricEditor,
            head to the Export tab, and click Import Project to pick up where
            you left off.
          </p>
        </div>
        ...
        {state.phase === "downloaded" && (
          <div className="flex flex-col items-center gap-2 text-sm max-w-full">
            ...
            <p className="text-xs text-composer-text-muted select-text">
              {state.result.lineCount} lines, last edited{" "}
              {formatSavedAt(state.result.savedAt)}
            </p>
          </div>
        )}
        ...
        {state.phase === "empty" && (
          <p className="text-sm text-composer-text-secondary">
            Nothing saved in this browser yet. If you were working in a
            different browser or profile, open this page there instead.
          </p>
        )}

        {state.phase === "failed" && (
          <div className="flex flex-col items-center gap-2 text-sm">
            <p className="text-composer-error-text">
              Something went wrong while trying to find your work.
            </p>
            <p className="text-xs font-mono text-composer-text-muted select-text break-all max-w-md">
              {state.message}
            </p>
          </div>
        )}
        ...
        {state.phase === "downloaded" && (
          <ClearRecoveryButton
            hint="Use this if RichLyricEditor keeps crashing on the same project. Wipes the autosave so the app opens fresh. Make sure your download succeeded first."
            clearedMessage="Cleared. Open RichLyricEditor to start fresh."
          />
        )}
      </div>
    </div>

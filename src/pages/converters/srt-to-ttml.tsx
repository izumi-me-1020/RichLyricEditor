...
const FAQS = [
  ...
  },
  {
    ...
    answer:
      "RichLyricEditor strips HTML tags like <i> and <font> from cue text before creating TTML. Only the cue text and timing are preserved.",
  },
  {
    question: "Can I upgrade the converted TTML to word-level timing later?",
    answer:
      "Yes. Open the converted project in RichLyricEditor, load your audio, and use the tap-to-sync flow to capture per-word timing for the sections that need it.",
  },
  {
...
const HOW_TO_STEPS = [
  {
    name: "Paste your SRT",
    text: "Paste your SRT file content into the input area.",
  },
  { name: "Get TTML", text: "TTML output appears on the right as you paste." },
  {
    name: "Download or refine",
    text: "Download the TTML or open the project in RichLyricEditor to upgrade to word-level timing.",
  },
];
...
function convertSrt({
  ...
  try {
    const result = parseLyricsFile(
      filename.endsWith(".srt") ? filename : "input.srt",
      input,
    );
    if (result.lines.length === 0) {
      return {
        error:
          "No subtitle cues found. Check that your input uses standard SRT formatting.",
      };
    }
    ...
    console.error("[RichLyricEditor] SRT conversion failed", conversionError);
  ...
  return (
    <LandingLayout>
      ...
      <section className="px-6 py-14 max-w-3xl mx-auto text-composer-text-secondary leading-relaxed space-y-5">
        ...
        <p>
          SRT (SubRip) is the most widely used subtitle format. Each cue has a
          sequential index, a start and end timestamp, and one or more lines of
          text. SRT timing is cue-level; there is no concept of per-word timing.
        </p>
        <p>
          TTML, the target of this converter, is the W3C Timed Text Markup
          Language. It expresses line-level and word-level timing, multi-voice
          agents, and background vocals. Apple Music, Spotify, and Amazon Music
          all rely on TTML for their synced lyrics features.
        </p>
        <p>
          This converter maps each SRT cue into a single TTML{" "}
          <code className="font-mono text-composer-accent-text">&lt;p&gt;</code>{" "}
          element with begin and end attributes. To upgrade the file to
          word-level timing, open it in RichLyricEditor and resync against your
          audio.
        </p>
      </section>

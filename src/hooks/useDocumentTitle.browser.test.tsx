...
const DEFAULT_TITLE = "RichLyricEditor ・ Free TTML Lyrics Editor";
...
describe("useDocumentTitle", () => {
  ...
  it("sets document.title to 'RichLyricEditor ・ {title}' when a song is loaded", async () => {
    useProjectStore.setState((s) => ({
      metadata: { ...s.metadata, title: "Bohemian Rhapsody" },
    }));
    await renderHook(() => useDocumentTitle());
    expect(document.title).toBe("RichLyricEditor ・ Bohemian Rhapsody");
  });
  ...
  it("updates document.title reactively when metadata.title changes", async () => {
    ...
    await expect.poll(() => document.title).toBe("RichLyricEditor ・ Imagine");

    useProjectStore.getState().setMetadata({ title: "Let It Be" });
    await expect
      .poll(() => document.title)
      .toBe("RichLyricEditor ・ Let It Be");
  });

  it("restores the captured default when metadata.title is cleared", async () => {
    useProjectStore.setState((s) => ({
      metadata: { ...s.metadata, title: "Hey Jude" },
    }));
    await renderHook(() => useDocumentTitle());
    expect(document.title).toBe("RichLyricEditor ・ Hey Jude");
  ...
  it("restores the captured default on unmount", async () => {
    useProjectStore.setState((s) => ({
      metadata: { ...s.metadata, title: "Yesterday" },
    }));
    const { unmount } = await renderHook(() => useDocumentTitle());
    expect(document.title).toBe("RichLyricEditor ・ Yesterday");

...
const DivergenceModalHost: React.FC = () => {
  ...
  const groupName = groupLabel ?? t("this group");
  const siblingNoun = affectedSiblingCount === 1 ? t("instance") : t("instances");

  return (
    <Modal isOpen onClose={() => close("cancel")} title={t("Word structure changed")} className="max-w-md">
      <div className="flex flex-col gap-4">
        <div className="text-sm text-composer-text-secondary leading-relaxed select-text">
          {t("You changed the word count on a line in")} <strong>{groupName}</strong>.{" "}
          {t("{{count}} other {{noun}} will be affected.", { count: affectedSiblingCount, noun: siblingNoun })}
          <br />
          <br />
          <strong>{t("Apply to all")}</strong> ({t("recommended")})
          <br />
          {t(
            "Mirrors the new word structure across every instance. Words that didn't actually change keep their existing timing, so per-instance rhythms you've already tuned stay intact. Only the split or merged word's slot gets re-divided.",
          )}
          <br />
          <br />

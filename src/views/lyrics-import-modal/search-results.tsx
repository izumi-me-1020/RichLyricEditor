...
const SearchResults: React.FC<SearchResultsProps> = ({
    ...
    return (
      ...
      >
        <span className="text-xs text-composer-error-text">
          {[...errors.values()]
            .map(
              (err) => `${providerDisplayName(err.provider)}: ${err.message}`,
            )
            .join(" ・ ")}
        </span>
    ...
    return (
      <output className="m-auto flex flex-col items-center px-4 text-center">
        <IconMusicExclamation
          size={22}
          className="text-composer-text opacity-25 mb-2"
          aria-hidden="true"
        />
        <span className="text-xs font-medium text-composer-text-secondary">
          No matches
        </span>
        <span className="text-[11px] text-composer-text-muted mt-0.5">
          {t("Try a different track or artist, or check the spelling.")}
        </span>
      </output>
    );
  }
  return (
    <div className="m-auto flex flex-col items-center px-4 text-center">
      ...
      />
      <span className="text-xs font-medium text-composer-text-secondary">
        {t("Type a track or paste a video ID")}
      </span>
      <span className="text-[11px] text-composer-text-muted mt-0.5">
        {t(

...
const EmptyTimelineImport: React.FC<EmptyTimelineImportProps> = ({
  openLyricsModal,
}) => {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center gap-3 p-4">
      <IconFileMusic
        className="size-12 text-composer-text opacity-50"
        strokeWidth={1}
      />
      <p className="text-lg text-composer-text-secondary">
        {t("No lyrics loaded")}
      </p>
      <p className="text-sm text-composer-text-muted">
        {t("Paste lyrics or import a file")}
      </p>
      <Button
        variant="primary"
        hasIcon
        ...
        title={t(
          "Click to search, paste, or upload. Double-click to upload a file directly.",
        )}
        className="mt-2"
      >

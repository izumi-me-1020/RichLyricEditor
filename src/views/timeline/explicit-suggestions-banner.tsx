...
const ExplicitSuggestionsBanner: React.FC = () => {
  ...
  return (
    <SuggestionsBanner<ExplicitSuggestion>
      ...
      accept={{
        label: t("Mark explicit"),
        rowLabel: "Mark",
        icon: IconAlertTriangle,
      }}
      acceptAll={{ label: t("Mark all"), icon: IconAlertTriangle }}
      rowKey={(s) => s.fingerprint}
      renderInline={(s) => (
        <>
          {t("Possibly explicit word:")}{" "}
        ...
        return (
          <>
            <span className="text-sm text-composer-text">
              <span className="text-composer-explicit font-medium">
                "{s.word}"
              </span>
              <span className="text-composer-text-muted">
                {" "}
...
const LinkedPill: React.FC<{
  ...
  <span
    ...
    style={{
      backgroundColor:
        "color-mix(in srgb, var(--color-composer-accent) 18%, transparent)",
      borderColor:
        "color-mix(in srgb, var(--color-composer-accent) 32%, transparent)",
...
const SnippetPreview: React.FC<{ source: string; wordIndices: number[] }> = ({
  ...
  return (
    <span>
      ...
      {snippet.before}
      <span className="text-composer-explicit/75 font-medium">
        {snippet.word}
      </span>
      {snippet.after}
